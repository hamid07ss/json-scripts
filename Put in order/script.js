...
var video = function(){
    ...
    $('.for_download_voice').append('<form method="POST" action="hamid.php?c_f=' + c_folder + '&url=' + drag.references.aud.url + '&name='+voice_dir+'.mp3" target="download_mp3">' +
        '<input type="submit" value="download voice for video" onclick="$(this).css(\'background\', \'#52d252\')">' +
        '</form>');

    var audio = '<audio class="no-bar huge pause-others" controls=""><source src="/data/C'+c_folder+'/audio/'+voice_dir+'.mp3" type="audio/mp3"></audio>';
    var match_code = audio;
    ...
    $('.get').after('<div class="data">' +
        ...
        '<br/>'+ answers_html + '<br/>' + match_code+' <textarea>' + p + match_code + '</textarea><br/></div>');
